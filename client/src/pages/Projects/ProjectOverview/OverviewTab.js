...
const OverviewTab = () => {
    ...
    const { start_date,end_date, description } = location.state;
    // console.log(start_date);
    const startDate = new Date(start_date);
    const day = startDate.getDate();
    const month = startDate.getMonth() + 1; // months start from 0, so add 1 to get the actual month
    const year = startDate.getFullYear();
    const formattedStartDate = `${day}/${month}/${year}`;
// console.log(formattedStartDate); // output: 12/04/2023

const endDate = new Date(end_date);
const d = endDate.getDate();
const m = endDate.getMonth() + 1; // months start from 0, so add 1 to get the actual month
                                    ...
                                    <Row>
                                        ...
                                        <Col lg={3} sm={6}>
                                            <div>
                                                ...
                                                <h5 className="fs-15 mb-0">{formattedStartDate}</h5>
                                            </div>
                                        </Col>
                                        <Col lg={3} sm={6}>
                                            <div>
                                                <p className="mb-2 text-uppercase fw-medium">Due Date :</p>
                                                <h5 className="fs-15 mb-0">{formattedEndDate}</h5>
                                            </div>
                                        </Col>
                                        <Col lg={3} sm={6}>
                                            <div>
                                                <Link to={"bugs"}><button className="btn btn-danger add-btn me-1" href="bugs">View Bugs</button></Link>
